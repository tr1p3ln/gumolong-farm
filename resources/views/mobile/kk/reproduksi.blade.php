...
<!-- Tab Nav -->
<nav class="sticky z-40 flex w-full bg-white border-b border-gray-200 top-14">
...
<div id="tab-kelahiran" class="px-4 py-4 space-y-4">
  ...
  <div class="p-4 bg-white border border-gray-200 shadow-sm rounded-xl">
    ...
    <div class="grid grid-cols-2 gap-3 py-3 border-gray-100 border-y">
      <div>
  ...
  <div class="text-center py-10 text-[#6B7280]">
    <span class="text-4xl text-gray-300 material-symbols-outlined">child_care</span>
    <p class="mt-2 text-sm">Belum ada data kelahiran</p>
  </div>
  @endforelse
...
<!-- Tab: Perkawinan -->
<div id="tab-perkawinan" class="hidden px-4 py-4 space-y-4">
  ...
  @empty
  <div class="text-center py-10 text-[#6B7280]">
    <span class="text-4xl text-gray-300 material-symbols-outlined">family_history</span>
    <p class="mt-2 text-sm">Belum ada data perkawinan</p>
  </div>
  @endforelse
...
<!-- Tab: Kebuntingan -->
<div id="tab-kebuntingan" class="hidden px-4 py-4 space-y-4">
  ...
  <div class="bg-blue-50 border border-blue-100 rounded-xl p-3 flex gap-2.5">
    ...
    <p class="text-xs leading-relaxed text-blue-700">Status kebuntingan dikonfirmasi setelah 40–50 hari perkawinan via USG atau observasi fisik.</p>
  </div>
  ...
  <div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
    <div class="flex items-center justify-between p-4 bg-gray-50/50">
      ...
      <span class="px-2 py-1 text-xs font-bold rounded-lg text-amber-600 bg-amber-50">Menunggu Konfirmasi</span>
    </div>
    <div class="grid grid-cols-2 gap-3 p-3">
      <form method="POST" action="{{ route('kk.reproduksi.konfirmasi', $kb->kawin_id) }}">
        ...
        <button type="submit"
          ...
          <span class="text-base material-symbols-outlined">close</span>Tidak Bunting
        </button>
      </form>
      <form method="POST" action="{{ route('kk.reproduksi.konfirmasi', $kb->kawin_id) }}">
        @csrf
        ...
        <button type="submit"
          ...
          <span class="text-base material-symbols-outlined">check_circle</span>Bunting
        </button>
      </form>
    </div>
  </div>
  @empty
  <div class="text-center py-10 text-[#6B7280]">
    <span class="text-4xl text-gray-300 material-symbols-outlined">pregnant_woman</span>
    <p class="mt-2 text-sm">Tidak ada perkawinan menunggu konfirmasi</p>
  </div>
  @endforelse
...
<!-- Bottom Nav -->
<nav class="fixed bottom-0  w-full max-w-[390px] bg-white border-t border-gray-200 flex justify-around items-center h-16 z-50 shadow-[0_-2px_8px_rgba(0,0,0,0.06)] left-1/2 -translate-x-1/2">
  ...
  <a href="{{ route('kk.kesehatan') }}" class="flex flex-col items-center justify-center flex-1 gap-0.5 text-gray-500 py-2">
    <span class="text-2xl material-symbols-outlined">medical_services</span>
  ...
  <a href="{{ route('kk.reproduksi') }}" class="flex flex-col items-center justify-center flex-1 gap-0.5 bg-[#2E7D32] text-white py-2 mx-1 rounded-xl">
    <span class="text-2xl material-symbols-outlined" style="font-variation-settings:'FILL' 1">family_history</span>
  ...
  <a href="{{ route('kk.validasi-timbangan') }}" class="flex flex-col items-center justify-center flex-1 gap-0.5 text-gray-500 py-2">
    <span class="text-2xl material-symbols-outlined">fact_check</span>
