...
<div x-show="showEditKelahiran" x-cloak
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-xl flex flex-col overflow-hidden border border-outline-variant/30"
        ...
        <div class="px-6 py-5 border-b border-outline-variant/30 flex justify-between items-start">
            <div>
                <h2 class="text-xl font-bold text-on-surface">Edit Kelahiran</h2>
                <p class="text-[10px] font-mono text-outline uppercase tracking-wider mt-0.5" x-text="'ID: LHR-' + editKelahiranData.lahir_id"></p>
            </div>
            <button type="button" @click="showEditKelahiran = false" class="p-1.5 hover:bg-surface-container rounded-lg text-outline">
                <span class="material-symbols-outlined text-xl">close</span>
            </button>
        </div>

        <div class="bg-surface-container-low px-6 py-2 border-b border-outline-variant/30">
            <p class="text-[10px] font-bold text-outline uppercase tracking-widest">EDIT MODE — Data diisi dari kelahiran yang dipilih</p>
        </div>
        ...
        <form id="edit-kelahiran-form" method="POST" :action="'/reproduksi/' + editKelahiranData.kawin_id + '/kelahiran/' + editKelahiranData.lahir_id" class="overflow-y-auto flex-1">
            ...
            <div class="p-6 space-y-5">
                ...
                <div class="p-3.5 bg-surface-container-low border border-outline-variant rounded-xl">
                    <p class="text-[10px] font-bold text-outline uppercase mb-2">Perkawinan Terkait</p>
                    <p class="text-sm font-semibold text-on-surface" x-text="editKelahiranData.pejantan_id + ' × ' + editKelahiranData.indukan_id"></p>
                </div>
                <p class="text-xs text-outline italic text-center">Perkawinan terkait tidak dapat diubah.</p>
