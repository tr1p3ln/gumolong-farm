...
<div x-show="showKonfirmasi" x-cloak
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden border border-outline-variant/30"
        ...
        <div class="px-6 py-5 border-b border-outline-variant/30 flex justify-between items-start">
            <div>
                <h2 class="text-xl font-bold text-on-surface">Konfirmasi Kebuntingan</h2>
            </div>
            <button type="button" @click="showKonfirmasi = false" class="p-1.5 hover:bg-surface-container rounded-lg text-outline">
            ...
            <form method="POST" :action="'/reproduksi/' + konfirmasiData.kawin_id + '/konfirmasi'">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="p-4 bg-surface-container-low border border-outline-variant rounded-xl space-y-3">
                        <p class="text-[10px] font-bold text-outline uppercase tracking-widest">Data Perkawinan</p>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                <p class="text-[10px] text-outline uppercase font-bold">ID Kawin</p>
                                <p class="font-mono font-bold text-on-surface" x-text="'KWN-' + konfirmasiData.kawin_id"></p>
                            </div>
                            <div>
                                <p class="text-[10px] text-outline uppercase font-bold">Pasangan</p>
                                <p class="font-semibold text-on-surface" x-text="konfirmasiData.pejantan_id + ' × ' + konfirmasiData.indukan_id"></p>
                            </div>
                            <div>
                                <p class="text-[10px] text-outline uppercase font-bold">Tgl Kawin</p>
                                <p class="font-semibold text-on-surface" x-text="konfirmasiData.tanggal_perkawinan"></p>
                            </div>
                            <div>
                    ...
                    {{-- Hasil Konfirmasi --}}
                    <div class="space-y-2">
                        <label class="text-xs font-semibold text-on-surface">Hasil Konfirmasi <span class="text-error">*</span></label>
                        <div class="grid grid-cols-2 gap-3">
                            <label>
                                <input type="radio" name="hasil" value="bunting" required class="sr-only peer">
                                <div class="p-4 border-2 border-outline-variant rounded-xl cursor-pointer transition-all
                                            peer-checked:border-green-500 peer-checked:bg-green-50 hover:bg-surface-container-low text-center">
                                    ...
                                    <p class="font-bold text-sm text-on-surface">Bunting</p>
                                    <p class="text-[10px] text-outline mt-0.5">Indukan dinyatakan bunting</p>
                                </div>
                            </label>
                            <label>
                                <input type="radio" name="hasil" value="tidak_bunting" required class="sr-only peer">
                                <div class="p-4 border-2 border-outline-variant rounded-xl cursor-pointer transition-all
                                            peer-checked:border-outline peer-checked:bg-surface-container-low hover:bg-surface-container-low text-center">
                                    <span class="material-symbols-outlined text-3xl text-outline mb-1 block">cancel</span>
                                    <p class="font-bold text-sm text-on-surface">Tidak Bunting</p>
                                    <p class="text-[10px] text-outline mt-0.5">Perkawinan tidak berhasil</p>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-on-surface">Metode Konfirmasi <span class="text-error">*</span></label>
                    ...
                    {{-- Tanggal Konfirmasi --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-on-surface">Tanggal Konfirmasi <span class="text-error">*</span></label>
