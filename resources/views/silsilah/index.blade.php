...
════════════════════════════════════ --}}
        <div class="flex items-start justify-between mb-6">
            <div>
                <h1 class="text-3xl font-extrabold tracking-tight text-gray-900">Silsilah & Pedigree</h1>
...
════════════════════════════════════ --}}
        <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="p-5 bg-white border border-gray-200 rounded-xl">
                <label class="text-xs font-bold tracking-wider text-gray-500 uppercase">Total Domba dengan Silsilah</label>
                <p class="text-3xl font-black text-[#2E7D32] mt-2">{{ $dombas->total() }}</p>
                <p class="mt-1 text-xs text-gray-500">di database</p>
            </div>
            <div class="p-5 bg-white border border-gray-200 rounded-xl">
                <label class="text-xs font-bold tracking-wider text-gray-500 uppercase">Domba dengan Kedua Orang Tua</label>
                <p class="text-3xl font-black text-[#607F5B] mt-2">
                    {{ $dombas->getCollection()->filter(fn($d) => $d->induk_id && $d->ayah_id)->count() }}
                </p>
                <p class="mt-1 text-xs text-gray-500">halaman ini</p>
            </div>
            <div class="p-5 bg-white border border-gray-200 rounded-xl">
                <label class="text-xs font-bold tracking-wider text-gray-500 uppercase">Domba tanpa Data Silsilah</label>
     ...
     FILTER BAR
════════════════════════════════════ --}}
        <div class="p-4 mb-6 bg-white border border-gray-200 rounded-xl">
            <form action="{{ route('silsilah.index') }}" method="GET" class="flex gap-3">
                ...
                <select name="kategori"
                    class="bg-white border border-gray-200 rounded-lg text-sm py-2 px-3 w-36
                ...
                <a href="{{ route('silsilah.index') }}"
                    class="px-4 py-2 text-sm font-bold text-gray-600 transition-colors border border-gray-300 rounded-lg hover:bg-gray-50">
                    Reset
                </a>
            </form>
        </div>
        ...
        <div class="overflow-hidden bg-white border border-gray-200 rounded-xl">
            <table class="w-full text-left">
                <thead class="border-b border-gray-200 bg-gray-50">
                    <tr>
                        <th class="px-6 py-4 text-xs font-bold tracking-wider text-gray-500 uppercase">Ear Tag / Domba</th>
                        <th class="px-6 py-4 text-xs font-bold tracking-wider text-gray-500 uppercase">Induk (Ibu)</th>
                        <th class="px-6 py-4 text-xs font-bold tracking-wider text-gray-500 uppercase">Pejantan (Ayah)</th>
                        <th class="px-6 py-4 text-xs font-bold tracking-wider text-gray-500 uppercase">Kategori</th>
                        <th class="px-6 py-4 text-xs font-bold tracking-wider text-gray-500 uppercase">Kelengkapan Silsilah
                        </th>
                        <th class="px-6 py-4 text-xs font-bold tracking-wider text-right text-gray-500 uppercase">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($dombas as $domba)
                        <tr class="transition-colors hover:bg-gray-50">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p class="font-mono text-sm font-bold text-gray-800">
                                                {{ $domba->induk->ear_tag_id }}</p>
                                            <p class="text-xs text-gray-500">{{ $domba->induk->nama ?? '-' }}</p>
                                        </div>
                                    </div>
                                @else
                                    <span class="text-xs italic text-gray-400">Tidak diketahui</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-5">
                                ...
                                @if ($hasInduk && $hasAyah)
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 bg-green-50 border border-green-200 text-green-700 text-xs font-bold rounded-full">
                                        ✓ Lengkap
                                    </span>
                                @elseif($hasInduk || $hasAyah)
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 border border-amber-200 text-amber-700 text-xs font-bold rounded-full">
                                        ⚠ Sebagian
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 border border-gray-200 text-gray-500 text-xs font-bold rounded-full">
                                        — Tidak Ada
                                    </span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 italic text-center text-gray-400">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50">
                ...
                {{-- Penambahan withQueryString() agar filter kategori tidak reset saat pindah halaman --}}
                {{ $dombas->withQueryString()->links() }}
            </div>
        </div>
        ...
        <div class="w-full max-w-2xl overflow-hidden bg-white shadow-2xl rounded-xl">
            <div class="flex items-start justify-between px-8 py-6 border-b border-gray-200">
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Periksa hubungan kekerabatan antara induk dan pejantan</p>
                </div>
            ...
            <div class="p-8 space-y-6">
                <div class="bg-[#FAFAF7] border-l-4 border-[#2E7D32] p-4 flex gap-3">
                    <svg class="w-5 h-5 text-[#2E7D32] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div x-show="cekResult" class="space-y-4">
                    <div :class="cekResult?.aman ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'"
                        class="p-6 text-center border-2 rounded-xl">
                        <p class="mb-2 text-xs font-bold tracking-widest text-gray-500 uppercase">
                            Inbreeding Coefficient (COI)
                        </p>
                        <p class="mb-2 text-5xl font-black" :class="cekResult?.aman ? 'text-[#2E7D32]' : 'text-[#B14B6F]'"
                    ...
                    <div class="p-4 border border-gray-200 rounded-lg bg-gray-50">
                        <p class="mb-2 text-xs font-bold tracking-wider text-gray-500 uppercase">Rekomendasi</p>
                    ...
                    <div class="grid grid-cols-4 gap-2 text-xs text-center">
                        ...
                        <div class="p-2 border rounded bg-amber-50 border-amber-200">
...
════════════════════════════════════ --}}
    <div x-show="modalRekomendasiPejantan" @keydown.escape.window="modalRekomendasiPejantan = false"
        class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" style="display: none;">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden max-h-[90vh] flex flex-col">
            <div class="flex items-start justify-between flex-shrink-0 px-8 py-6 border-b border-gray-200">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">Rekomendasi Pejantan</h2>
                    <p class="mt-1 text-xs text-gray-500">Pejantan diurutkan dari COI terkecil (terbaik) ke terbesar</p>
                </div>
                ...
                <div x-show="rekResult">
                    ...
                    <div class="overflow-hidden border border-gray-200 rounded-xl">
                        <table class="w-full text-sm text-left">
                            <thead class="border-b border-gray-200 bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">#</th>
                                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">Pejantan
                                    </th>
                                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">Ras</th>
                                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">Kandang
                                    </th>
                                    <th
                                        class="px-4 py-3 text-xs font-bold tracking-wider text-center text-gray-500 uppercase">
                                        COI</th>
                                    <th
                                        class="px-4 py-3 text-xs font-bold tracking-wider text-center text-gray-500 uppercase">
                                        Status</th>
